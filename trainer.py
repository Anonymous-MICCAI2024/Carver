...
from utils import dfs_remove_weight, hdf5_saver
...
class SemanticSeg(object):
    ...
    '''
    def __init__(self,net_name=None,encoder_name=None,lr=1e-3,n_epoch=1,channels=2,num_classes=1,roi_number=1,scale=None, input_shape=None,crop=48,
    ...
    def trainer(self,train_path,val_path,cur_fold,output_dir=None,log_dir=None,optimizer='Adam',
        ...
        if len(self.input_shape) > 2:
            train_transformer = transforms.Compose([
                ...
                RandomTranslationRotationZoom3D(mode='r',num_class=self.num_classes),
    ...
    def inference(self,test_path,net=None):
        ...
        with torch.no_grad():
            for step,sample in enumerate(test_loader):
                ...
                # measure run dice  
                output = torch.argmax(torch.softmax(output,dim=1),1).detach().cpu().numpy().squeeze()  #N*D*H*W 
                target = torch.argmax(target,1).detach().cpu().numpy().squeeze() #N*D*H*W
                data = data.detach().cpu().numpy().squeeze()
                # # save output matrix
                unet_path = '/staff/ydli/projects/Med_Seg/seg/new_inf/kbr/v2_r0.9.2/fold5/hdf5_eval/'
                if not os.path.exists(unet_path):
                    os.makedirs(unet_path)
                # print("output type: ", type(output))
                # print("output shape: ", output.shape)
                hdf5_path = unet_path + '/' + os.path.basename(test_path[step])
                hdf5_saver(hdf5_path, data, output, target)   
    ...
    def _get_net(self,net_name):
        ...
        if net_name == 'unet_3d':
        ...
        elif net_name == 'se_res_unet':
            from model.da_unet import se_res_unet
            net = se_res_unet(init_depth=self.input_shape[0],n_channels=self.channels,n_classes=self.num_classes)
        
        elif net_name == 'da_se_unet':
            from model.da_unet import da_se_unet
            net = da_se_unet(init_depth=self.input_shape[0],n_channels=self.channels,n_classes=self.num_classes)
        
        elif net_name == 'res_da_se_unet':
            from model.da_unet import res_da_se_unet
            net = res_da_se_unet(init_depth=self.input_shape[0],n_channels=self.channels,n_classes=self.num_classes)
            ...
            from model.vnet import vnet_res_da_se
            net = vnet_res_da_se(init_depth=self.input_shape[0],in_channels=self.channels,classes=self.num_classes)
        
        elif net_name == 'vnet':
            from model.vnet import vnet
            net = vnet(init_depth=self.input_shape[0],in_channels=self.channels,classes=self.num_classes)

        elif net_name == 'vnet_res':
            from model.vnet import vnet_res
            net = vnet_res(init_depth=self.input_shape[0],in_channels=self.channels,classes=self.num_classes)

        elif net_name == 'vnet_se':
            from model.vnet import vnet_se
            net = vnet_se(init_depth=self.input_shape[0],in_channels=self.channels,classes=self.num_classes)

        elif net_name == 'vnet_da':
            from model.vnet import vnet_da
            net = vnet_da(init_depth=self.input_shape[0],in_channels=self.channels,classes=self.num_classes)

        elif net_name == 'vnet_res_se':
            from model.vnet import vnet_res_se
            net = vnet_res_se(init_depth=self.input_shape[0],in_channels=self.channels,classes=self.num_classes)
