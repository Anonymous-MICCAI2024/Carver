...
def hdf5_saver(data_path, data, outmatrix, target):
    hdf5_file = h5py.File(data_path, 'w')
    hdf5_file.create_dataset('inf_image', data=outmatrix.astype(np.int16))
    hdf5_file.create_dataset('gt_label', data=target.astype(np.int16))
    hdf5_file.create_dataset('data', data=data.astype(np.int16))
    hdf5_file.close() 
