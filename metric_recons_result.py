...
def read_xml_file(gt_name, mode, sid):
    ...
    # print(hdf5_file)
    matrix = hdf5_reader(hdf5_file,'label')
...
def get_sid_efs(sid ,ed_pred_volume, ed_gt_volume, pred_filepath, gt_filepath):
    sid = sid.split('_')[2]
    sid = 'SID_3042_'+sid

    sid_mode = sid+'_es'
    pred_name = 'kbr_es_heart_'+sid
    ...
    if os.path.exists(pred_file):
        ...
        # print(gt_name)
        gt_points, gt_matrix = read_xml_file(gt_file, 'es', sid_mode)
...
if __name__ == '__main__':
    ...
    pred_filepath = '/staff/ydli/projects/OReX/output/directory/kbr_fold3_r0'
    gt_filepath = '/staff/ydli/projects/OReX/Data/kbr_patient_backup'
    mode_list = ['ed', 'es']
        ...
        for pred_name in os.listdir(pred_filepath):
            if mode in pred_name:
                ...
                try:
                    ...
                    if 'ed' in mode and '3042' in sid:
                        print('3042 sid:', sid)
                        ed_pred_volume = np.sum(preds_matrix)
                        ed_gt_volume = np.sum(gt_matrix)
                        efs = get_sid_efs(sid ,ed_pred_volume, ed_gt_volume, pred_filepath, gt_filepath)
                    else:
                        efs = 0
    ...
    csv_dict = {'sid_mode':sid_list, 'pred_obj':pred_filelist, 'gt_xml':gt_filelist, 
                ...
                'jaccard':jaccard, 'VolumeSimilarity':volume_sim, 'FalseNegativeError':false_negative, 'FalsePositiveError':false_positive, 'asd':asd_list, 'efs_sim':efs_sim}
    df = pd.DataFrame(csv_dict)
    df.to_csv('/staff/ydli/projects/OReX/output/directory/kbr/metrics_orex_fold3_r5.csv')
